...
from telegram.ext import _dictpersistence

CDCData = Tuple[List[Tuple[str, float, Dict[str, Any]]], Dict[str, str]]

# Monkey patch dict persistence to use pickle module instead of json module
_dictpersistence.json = pickle
DictPersistence = _dictpersistence.DictPersistence


class SQLAlchemyPersistence(DictPersistence):
    """Using SQLAlchemy database to make user/chat/bot data persistent across reboots.

    Args:
        url (:obj:`str`, Optional) the database url.
        session (:obj:`scoped_session`, Optional): sqlalchemy scoped session.
        on_flush (:obj:`bool`, optional): if set to :obj:`True` :class:`Persistence`
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            data = pickle.loads(data_[0]) if data_ is not None else {}

            chat_data_json = data.get("chat_data")
            user_data_json = data.get("user_data")
            bot_data_json = data.get("bot_data")
            conversations_json = data.get("conversations", pickle.dumps({}))
            ...
            if not data:
                insert_qry = "INSERT INTO persistence (data) VALUES (:pickledata)"
                self._session.execute(
                    text(insert_qry), {"pickledata": pickle.dumps({})}
                )
    ...
    def _dump_into_pickle(self) -> Any:
    ...
    def _update_database(self) -> None:
        ...
        try:
            insert_qry = "UPDATE persistence SET data = :pickledata"
            params = {"pickledata": self._dump_into_pickle()}
